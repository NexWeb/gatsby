...
const runFilterAndSort = (args: Object) => {
  ...
  let result = applyFilters(filter, firstOnly, nodeTypeNames, resolvedFields)
 ...
 */
const applyFilters = (filter, firstOnly, nodeTypeNames, resolvedFields) => {
  let result = filterWithoutSift(filter, nodeTypeNames)
  if (result) {
    if (firstOnly) {
...
const filterWithoutSift = (filter, nodeTypeNames) => {
  ...
  return runFlatFilterWithoutSift(chainWithNeedle, targetValue, nodeTypeNames)
}
